...
(function (a) {
    a.extend(a.jqx._jqxGrid.prototype, {_updatefilterrowui: function (f) {
    ...
    }, _addfilterwidget: function (B, d, z) {
        var F = this.that;
        var y = "";
        for (var D = 0; D < F.dataview.filters.length; D++) {
            var w = F.dataview.filters[D];
            if (w.datafield && w.datafield == B.datafield) {
                y = w.filter.getfilters()[0].value;
                break
            }
        }
        var g = function (G, H) {
            ...
            f.addClass(G.toThemeProperty("jqx-widget"));
            f.addClass(G.toThemeProperty("jqx-input"));
            f.addClass(G.toThemeProperty("jqx-rc-all"));
            f.addClass(G.toThemeProperty("jqx-widget-content"));
            if (G.rtl) {
                f.css("direction", "rtl")
            }
            f.appendTo(H);
            f.width(z - 10);
            f.height(G.filterrowheight - 10);
            f.css("margin", "4px");
            if (B.createfilterwidget) {
                B.createfilterwidget(B, H, f)
            }
            B._filterwidget = f;
            f.focus(function () {
                ...
                G.focusedfilter = f;
                f.addClass(G.toThemeProperty("jqx-fill-state-focus"));
                return false
            });
            f.blur(function () {
                f.removeClass(G.toThemeProperty("jqx-fill-state-focus"))
            });
            f.keydown(function (I) {
                ...
                f[0]._writeTimer = setTimeout(function () {
                    if (!G._loading) {
                        if (G["_oldWriteText" + f[0].id] != f.val()) {
                            G._applyfilterfromfilterrow();
                            G["_oldWriteText" + f[0].id] = f.val()
                        }
                    }
                }, 800);
                G.focusedfilter = f
            });
            G.host.removeClass("jqx-disableselect");
            G.content.removeClass("jqx-disableselect");
            f.val(y)
        };
        if (B.datatype != null) {
            if (B.filtertype == "number") {
                if (B.datatype == "string" || B.datatype == "date" || B.datatype == "bool") {
                    B.filtertype = "textbox"
                }
            }
            if (B.filtertype == "date") {
                if (B.datatype == "string" || B.datatype == "number" || B.datatype == "bool") {
                    B.filtertype = "textbox"
                }
            }
            if (B.filtertype == "bool") {
                if (B.datatype == "string" || B.datatype == "number" || B.datatype == "date") {
                    B.filtertype = "textbox"
                }
            }
        }
        switch (B.filtertype) {
            case"number":
                ...
                d.append(m);
                var z = d.width() - 20;
                var r = function (H, I, f) {
                    var G = a('<input style="float: left;" autocomplete="off" type="textarea"/>');
                    if (F.rtl) {
                        G.css("float", "right");
                        G.css("direction", "rtl")
                    }
                    G[0].id = a.jqx.utilities.createId();
                    G.addClass(F.toThemeProperty("jqx-widget"));
                    G.addClass(F.toThemeProperty("jqx-input"));
                    G.addClass(F.toThemeProperty("jqx-rc-all"));
                    G.addClass(F.toThemeProperty("jqx-widget-content"));
                    G.appendTo(H);
                    G.width(I - 10);
                    G.height(F.filterrowheight - 10);
                    G.css("margin", "4px");
                    G.css("margin-right", "2px");
                    G.focus(function () {
                        F.focusedfilter = G;
                        G.addClass(F.toThemeProperty("jqx-fill-state-focus"))
                    });
                    G.blur(function () {
                        G.removeClass(F.toThemeProperty("jqx-fill-state-focus"))
                    });
                    G.keydown(function (J) {
                        if (J.keyCode == "13") {
                            F._applyfilterfromfilterrow()
                        }
                        if (G[0]._writeTimer) {
                            clearTimeout(G[0]._writeTimer)
                        }
                        G[0]._writeTimer = setTimeout(function () {
                            if (!F._loading) {
                                if (F["_oldWriteText" + G[0].id] != G.val()) {
                                    F._applyfilterfromfilterrow();
                                    F["_oldWriteText" + G[0].id] = G.val()
                                }
                            }
                        }, 800);
                        F.focusedfilter = G
                ...
                var s = a("<div class='filter' style='float: left;'></div>");
                s.css("margin-top", "4px");
                s.appendTo(m);
                if (F.rtl) {
                    s.css("float", "right")
                }
                var h = 0;
                if (B.filtercondition != null) {
                    var e = A.indexOf(B.filtercondition);
                    if (e != -1) {
                        h = e
                    }
                }
                s.jqxDropDownList({touchMode: F.touchmode, rtl: F.rtl, dropDownHorizontalAlignment: "right", enableBrowserBoundsDetection: true, selectedIndex: h, width: 18, height: 21, dropDownHeight: 150, dropDownWidth: 170, source: A, theme: F.theme});
                s.jqxDropDownList({selectionRenderer: function (f) {
                    return""
                }});
                s.jqxDropDownList("setContent", "");
                s.find(".jqx-dropdownlist-content").hide();
                if (B.createfilterwidget) {
                    B.createfilterwidget(B, d, m)
                }
                B._filterwidget = m;
                var j = null;
                this.addHandler(s, "select", function () {
                    var f = s.jqxDropDownList("getSelectedItem").label;
                    if (B._filterwidget.find("input").val().length > 0 && !F.refreshingfilter) {
                        F._applyfilterfromfilterrow()
                    } else {
                        if (B._filterwidget.find("input").val().length == 0 && !F.refreshingfilter) {
            ...
            case"date":
                if (this.host.jqxDateTimeInput) {
                    ...
                    b.jqxDateTimeInput({localization: n, rtl: F.rtl, showFooter: true, formatString: B.cellsformat, selectionMode: "range", value: null, theme: this.theme, width: z - 10, height: this.filterrowheight - 10});
                    if (B.createfilterwidget) {
                        B.createfilterwidget(B, d, b)
                    }
                    B._filterwidget = b;
                    this.addHandler(b, "valuechanged", function (f) {
                        if (!F.refreshingfilter) {
                            F._applyfilterfromfilterrow();
                            F.focusedfilter = null
                        }
                    })
                } else {
                    g(this, d)
            ...
            case"checkedlist":
                if (this.host.jqxDropDownList) {
                    var q = this._getfilterdataadapter(B);
                    var l = false;
                    var s = a("<div></div>");
                    s.css("margin", "4px");
                    var t = B.datafield;
                    var u = B.filtertype == "checkedlist" ? true : false;
                    var C = z < 150 ? 220 : "auto";
                    q.dataBind();
                    var p = q.records;
                    var k = p.length < 8 ? true : false;
                    l = k;
                    s.appendTo(d);
                    s.jqxDropDownList({touchMode: F.touchmode, rtl: F.rtl, checkboxes: u, dropDownWidth: C, source: q.records, autoDropDownHeight: k, theme: this.theme, width: z - 10, height: this.filterrowheight - 10, displayMember: B.displayfield, valueMember: t});
                    var c = s.jqxDropDownList("listBox");
                    if (u) {
                        s.jqxDropDownList({selectionRenderer: function () {
                            return F.gridlocalization.filterselectstring
                        }});
                        var x = a('<span style="top: 2px; position: relative; color: inherit; border: none; background-color: transparent;">' + F.gridlocalization.filterselectstring + "</span>");
                        x.addClass(this.toThemeProperty("jqx-item"));
                        if (c != undefined) {
                            if (!l) {
                                c.host.height(200)
                            }
                            c.insertAt(F.gridlocalization.filterselectallstring, 0);
                            s.jqxDropDownList("setContent", x);
                            var i = true;
                            var E = new Array();
                            c.checkAll(false);
                            F.addHandler(c.host, "checkChange", function (H) {
                                s[0]._selectionChanged = true;
                                if (!i) {
                                    return
                                }
                                if (H.args.label != F.gridlocalization.filterselectallstring) {
                                    i = false;
                                    c.host.jqxListBox("checkIndex", 0, true, false);
                                    var f = c.host.jqxListBox("getCheckedItems");
                                    var G = c.host.jqxListBox("getItems");
                                    if (f.length == 1) {
                        ...
                        s.jqxDropDownList({selectedIndex: 0})
                    }
                    if (B.createfilterwidget) {
                        B.createfilterwidget(B, d, s)
                    }
                    B._filterwidget = s;
                    var o = s.jqxDropDownList("dropdownlistWrapper");
                    if (B.filtertype == "list") {
                        this.addHandler(s, "select", function (f) {
                            if (!F.refreshingfilter) {
                        ...
                        this.addHandler(s, "close", function (f) {
                            if (s[0]._selectionChanged) {
                                F._applyfilterfromfilterrow();
                                F.focusedfilter = null;
                                s[0]._selectionChanged = false
                            }
                        })
                    }
                } else {
            ...
            case"boolean":
                if (this.host.jqxCheckBox) {
                    var v = a('<div tabIndex=0 style="opacity: 0.99; position: absolute; top: 50%; left: 50%; margin-top: -7px; margin-left: -10px;"></div>');
                    v.appendTo(d);
                    v.jqxCheckBox({enableContainerClick: false, animationShowDelay: 0, animationHideDelay: 0, hasThreeStates: true, theme: this.theme, checked: null});
                    if (B.createfilterwidget) {
                        B.createfilterwidget(B, d, v)
                    }
                    if (y === true || y == "true") {
                        v.jqxCheckBox({checked: true})
                    } else {
                        if (y === false || y == "false") {
                    ...
                    B._filterwidget = v;
                    this.addHandler(v, "change", function (f) {
                        if (!F.refreshingfilter) {
                            if (f.args) {
                                F.focusedfilter = null;
                                F._applyfilterfromfilterrow()
