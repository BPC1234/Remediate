...
(function (a) {
    ...
    a.extend(a.jqx._jqxDateTimeInput.prototype, {defineInstance: function () {
        ...
        var b = a("<div class='jqx-max-size jqx-position-relative'></div>").appendTo(this.host);
        this.dateTimeInput = a("<input class='jqx-position-absolute' id='input" + this.element.id + "' autocomplete='off' type='textarea'/>").appendTo(b);
        ...
        this.calendarButton = a("<div style='height: 100%;' class='jqx-position-absolute'><div></div></div>").appendTo(b);
        if (!this.rtl) {
        ...
        this.calendarButtonIcon = a(this.calendarButton.children()[0]);
        ...
        if (e == 0) {
            e = parseInt(this.input.css("font-size")) + 3
        }
        ...
        this.addHandler(this.dateTimeInput, "blur", function (j) {
            if (d.value != null) {
                d.isEditing = false;
                var h = d.value.dateTime.getDay();
                var g = d._oldDT;
                d._validateValue();
                if (d.selectableDays.length != 7) {
                    var l = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];
                    var k = d.value.dateTime.getDay();
                    var f = l[k];
                    if (d.selectableDays.length > 0) {
                        if (d.selectableDays.indexOf(f) == -1) {
                            if (d.value.dateTime < g) {
                                while (d.selectableDays.indexOf(f) == -1) {
                                    d.value._addDays(-1);
                                    var k = d.value.dateTime.getDay();
                                    f = l[k]
                                }
                            ...
                            d.value.day = d.value.dateTime.getDate();
                            d.value.month = d.value.dateTime.getMonth() + 1;
                            d._updateEditorsValue()
                        }
                    }
                }
                d._updateText();
        ...
        var c = null;
        var t = 0;
        for (var w = 0; w < this.items.length; w++) {
            switch (this.items[w].type) {
                case"FORMAT_AMPM":
                    f = this.editors[w].value;
                    c = this.editors[w];
                    break;
                case"Character":
                    break;
                case"Day":
                    if (this.items[w].format.length < 4) {
                        v = this.editors[w].value;
                        p[t++] = this.editors[w];
                        if (v == 0) {
                            v = 1
                        }
                    ...
                    break;
                case"FORMAT_hh":
                    var z = this.editors[w];
                    h = z.value;
                    break;
                case"FORMAT_HH":
                    h = this.editors[w].value;
                    break;
                case"Millisecond":
                    b = this.editors[w].value;
                    break;
                case"Minute":
                    q = this.editors[w].value;
                    break;
                case"Month":
                    C = this.editors[w].value;
                    n = true;
                    if (C == 0) {
                        C = 1
                    }
                ...
                case"ReadOnly":
                    break;
                case"Second":
                    D = this.editors[w].value;
                    break;
                case"Year":
                    j = true;
                    o = this.editors[w].value;
                    var B = this.editors[w].getDateTimeItem().format;
                    if (B.length < 3) {
                        var s = "1900";
                        if (s.Length == 4) {
                            var r = "" + s[0] + s[1];
        ...
        var A = this.value != null ? new Date(this.value.dateTime) : null;
        if (o > 0 && C > 0 && v > 0 && q >= 0 && h >= 0 && D >= 0 && b >= 0) {
            var E = this.value;
            if (E != null) {
                if (!j) {
                    o = E.year
            ...
            try {
                ...
                if (E._daysInMonth(o, C) <= v) {
                    v = E._daysInMonth(o, C);
                ...
                this.value._setYear(parseInt(o));
                this.value._setDay(v);
                this.value._setMonth(C);
                this.value._setHours(h);
                this.value._setMinutes(q);
                this.value._setSeconds(D);
                this.value._setMilliseconds(b)
            } catch (g) {
                this.value = E
            }
            if (A != null) {
                var u = this.value.dateTime.getFullYear() == A.getFullYear() && this.value.dateTime.getDate() == A.getDate() && this.value.dateTime.getMonth() == A.getMonth() && this.value.dateTime.getHours() == A.getHours() && this.value.dateTime.getMinutes() == A.getMinutes() && this.value.dateTime.getSeconds() == A.getSeconds();
                if (!u) {
                    this._raiseEvent("0", this.value.dateTime);
                    if (this.cookies) {
                        if (this.value != null) {
