...
$.extend($.expr[":"], {"contains-ci": function (c, b, a, d) {
    return(c.textContent || c.innerText || $(c).text() || "").toLowerCase().indexOf((a[3] || "").toLowerCase()) >= 0
}});
$(function () {
    var b = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"], d = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];
    var a = new Date();
    a.setDate(a.getDate());
    ...
    $("i.moreOptions, .tableSettings").popover({trigger: "click", placement: "bottom", html: true, content: function () {
...
function customTabAction(ext){
    $(ext+" ul.nav-tabs li a").click(function () {
...
function setCustomFileAction(fileId){
...
function setUploadButton(pathId, selectionBtnId, textFieldId ){
    $(  selectionBtnId ).click(function(e){
        $( pathId ).trigger("click");
    });

    $( pathId).change(function(e){
        var val = $(this).val();
        var file = val.split(/[\\/]/);
        $(textFieldId).val(file[file.length-1]);
    });
}