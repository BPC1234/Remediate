	...
	$.addFlex = function (t, p) {
		...
		if (p.colModel) { //create model if any
			...
			for (var i = 0; i < p.colModel.length; i++) {
                ...
                var hiddenInput = '<input type="hidden" id="hiddenId" value="'+cm.name+'">';
				$(th).append(hiddenInput);
				$(tr).append(th);
			}
			$(thead).append(tr);
			$(thead).append('<input type="hidden" id="jsonUrl" value="'+  (p.url).substr(0,(p.url).indexOf('?')> 0 ? (p.url).indexOf('?'): (p.url).length)+'">');
			$(t).prepend(thead);
		} // end if p.colmodel
		//init divs
		...
		if (p.buttons) {
			...
			for (var i = 0; i < p.buttons.length; i++) {
				...
				if (!btn.separator) {
					...
					btnDiv.innerHTML = ("<div><span>") + (btn.hidename ? "&nbsp;" : btn.bclass) + ("</span></div>");
...
function printTable(table, isflexigrid)
{
    $('#content').css('opacity','0.6');
    showLoadingImage();
    var headerList = getActiveTableHeader($(table).parent('div.bDiv').parent('div').children('div.hDiv').children('div.hDivBox').find('table'));
    var printColList = getActiveTableHeaderForColumn($(table).parent('div.bDiv').parent('div').children('div.hDiv').children('div.hDivBox').find('table'));
    var newTable = reloadForCSV(headerList,printColList,table);
    newWin = window.open("");
    newWin.document.write(newTable);
    newWin.print();
    newWin.close();
...
var toExcel = (function ()
{
    ...
    return function (table, isflexigrid)
    {
        $('#content').css('opacity','0.6');
        showLoadingImage();
        //var object = $("#dbColumnHeaderId").val();
        var headerList = getActiveTableHeader($(table).parent('div.bDiv').parent('div').children('div.hDiv').children('div.hDivBox').find('table'));
        var printColList = getActiveTableHeaderForColumn($(table).parent('div.bDiv').parent('div').children('div.hDiv').children('div.hDivBox').find('table'));
        var activeHeader = $(table).parent('div.bDiv').parent('div').children('div.hDiv').children('div.hDivBox').find('table').html();
        var newTable = reloadForCSV(headerList,printColList,table);
        var htmlTemplate = (newTable);
        var object = $('<div/>').html(htmlTemplate).contents();
        var ctx = { worksheet: name || 'Worksheet', table:$(object).parent().find('table#commonTable').html()}
        document.getElementById("dlink").href = uri + base64(format(template, ctx));
        document.getElementById("dlink").download = table.replace("#","").replace(".","")+".xls";
...
function tableToCSV(table){
        ...
        var htmlTemplate = (newTable);
        var object = $('<div/>').html(htmlTemplate).contents();
        var activeHeader = $(table).parent('div.bDiv').parent('div').children('div.hDiv').children('div.hDivBox').find('table');
        var csv = $($(object).parent().find('table#commonTable')).table2CSV({delivery:'value'});
        document.getElementById("dlink").href = 'data:text/csv;charset=UTF-8,'+ encodeURIComponent(csv);
        document.getElementById("dlink").download = table.replace("#","").replace(".","")+".csv";
        document.getElementById("dlink").click();
        hideLoadingImage();
        $('#content').css('opacity','1.0');
}

function reloadForCSV(headerList,columnList,table){
    var totalCount=0;
    var url2 = $(table).parent('div.bDiv').parent('div').children('div.hDiv').children('div.hDivBox')
        .find('table').children('thead').children('#jsonUrl').val()+'?&rp=max&page=1&query=&qtype=status&sortorder=asc'+globalExportParam;
    $.ajax({
        type: 'post',
        url: url2,
        async: false,
        success: function (totalCountFromResponse) {
            totalCount = totalCountFromResponse.total;
    ...
    $(".exportModal tr td #to").attr("data-max",totalCount);
    $(".exportModal tr td #to").attr("placeholder","max val="+totalCount);
    $(".exportModal tr td ul li.max").text("placeholder","This value should be lower than "+totalCount+".");
    var startPoint = 0;
        startPoint = $(".exportModal tr td #from").val();
        var endPoint = 0;
        endPoint = $(".exportModal tr td #to").val();
        var url = '../admin/getTrxSearchDataList.html?'
            +'page=1'
            +'&selColList='+headerList
            +'&printColList='+columnList
            +'&tableName='+table.replace('#','').replace('.','')
            +'&option=2'
            +'&rp=max';
        var objectData;
        $.ajax({
...
function getActiveTableHeader(tableIdOrClass){
    var visibleHeader = '';
    $(tableIdOrClass).find('tr th:visible').each(function() {
         visibleHeader = visibleHeader + (visibleHeader != ''? ',': '') + $(this).children('div').children('#hiddenId').val();
    });
    return visibleHeader;
}
function getActiveTableHeaderForColumn(tableIdOrClass){
    var visibleHeader = '';
    $(tableIdOrClass).find('tr th:visible').each(function() {
         visibleHeader = visibleHeader + (visibleHeader != ''? ',': '') + $(this).children('div').text();
    });
    return visibleHeader;
}

function resetTableData(table){
     $(table).flexOptions({newp: 1,query:''}).flexReload();
     if($(table).parent('div').parent('div').find('div.sDiv').is(":hidden") == false){
        $(table).parent('div').parent('div').find('div.pDiv').children('div').children('div').children('div').click();
     }
}
...
function sizeGrid(data) {
    ...
    var totalHiddenWidthCount  = 0;
    var totalWidth = data.$grid.innerWidth()-5; //5 padding - not found where this is set
    var fixedWidth = 0;
    var fluidCols = [];
    for(var i=0; i<data.grid.colModel.length; i++ ) {
        if( !isNaN(data.grid.colModel[i].width) ) {
            ...
            if(data.$table.find('tr:eq('+i+') td:eq('+i+')').css('display') == 'none'){
                totalHiddenWidthCount++;
            }
        } else {
            fluidCols.push(i);
        }
    ...
    var newWidth = (totalWidth-fixedWidth-(totalHiddenWidthCount*10))/fluidCols.length-10;
    //alert('totalFixedWidthCount ='+totalHiddenWidthCount+ ' totalWidth= '+totalWidth+' fixed width='+fixedWidth+' fluidCols.length='+fluidCols.length+' newWidth='+newWidth)
...
function groupByView(table){
    globalTable = table
    if ( $(table).parent('div.bDiv').parent('div').children('div#jqxWidget').length ) {
        //alert('div is exist');
        hideFlexigridAndShowGrouping(table);
    }else{
        //alert('not exist.');
        doWhenGroupingIsNotExist(table);
    }


}
function getTableWidthList(tableIdOrClass){
    var visibleHeaderWidth = '';
    $(tableIdOrClass).find('tr th:visible').each(function() {
        visibleHeaderWidth = visibleHeaderWidth + (visibleHeaderWidth != ''? ',': '') + $(this).innerWidth();
    });
    return visibleHeaderWidth;
}

function setActionOnCrossButton(){
    var count = $("#jqxWidget #groupTable div.jqx-position-relative div div#groupsheader div").size();
    if(count == 4){
        hideGroupingAndShowFlexigrid(globalTable);
    }
}

function doWhenGroupingIsNotExist(table){
    var totalWidth = $(table).parent('div.bDiv').parent('div').children('div.hDiv').children('div.hDivBox').find('table').outerWidth();
    var widthList = getTableWidthList($(table).parent('div.bDiv').parent('div').children('div.hDiv').children('div.hDivBox').find('table'));
    var headerList = getActiveTableHeader($(table).parent('div.bDiv').parent('div').children('div.hDiv').children('div.hDivBox').find('table'));
    var columnList = getActiveTableHeaderForColumn($(table).parent('div.bDiv').parent('div').children('div.hDiv').children('div.hDivBox').find('table'));
    var url2 = $(table).parent('div.bDiv').parent('div').children('div.hDiv').children('div.hDivBox')
        .find('table').children('thead').children('#jsonUrl').val()+'?&rp=max&page=1&query=&qtype=status&sortorder=asc'+globalExportParam;
    $.ajax({
        type: 'post',
    ...
    $.ajax({
        ...
        async: false,
        success: function (data) {
            var dataDiv = $('<div id="bDivCopy">' + data + '</div>');
            hideFlexigridAndShowGrouping(table);
            $(table).parent('div.bDiv').parent('div').children('.bDiv').after(dataDiv.filter('#bDivCopy').html());
            globalTable = table ;
        }
    });
}
function hideFlexigridAndShowGrouping(table){
    $(table).parent('div.bDiv').parent('div').children('div.tDiv').hide();
    $(table).parent('div.bDiv').parent('div').children('div.bDiv').hide();
    $(table).parent('div.bDiv').parent('div').children('div.pDiv').hide();
    $(table).parent('div.bDiv').parent('div').children('div.hDiv').hide();
    $(table).parent('div.bDiv').parent('div').children('div#jqxWidget').show();
}
function hideGroupingAndShowFlexigrid(table){
    $(table).parent('div.bDiv').parent('div').children('div#jqxWidget').remove();
    $(table).parent('div.bDiv').parent('div').children('div.tDiv').show();
    $(table).parent('div.bDiv').parent('div').children('div.bDiv').show();
    $(table).parent('div.bDiv').parent('div').children('div.pDiv').show();
    $(table).parent('div.bDiv').parent('div').children('div.hDiv').show();
}
