...
//    key = KeyGenerator.getInstance("DES").generateKey();
    ...
    public static void setGlobalLoginRoleName(HttpServletRequest request, String message){

        request.getSession().setAttribute(Constants.GLOBAL_LOGIN_ROLE, message);
    }

    public static String getGlobalLoginRoleName(HttpServletRequest request){
        if(request.getSession().getAttribute(Constants.GLOBAL_LOGIN_ROLE) != null){
            String message = (String)request.getSession().getAttribute(Constants.GLOBAL_LOGIN_ROLE);
            return message;
        }
        return "";
    }
...
*/
    ...
    public static byte[] generateBarcode(String msg) throws Exception {
        ...
        int height = symbol.getHeight() - fontSize;
        ...
        java.io.ByteArrayInputStream inp = new java.io.ByteArrayInputStream(out.toByteArray());
        BufferedImage originalImage = javax.imageio.ImageIO.read(inp);
        int scaledHeight = originalImage.getHeight();
        int scaledWidth = originalImage.getWidth();
        scaledHeight = (int) (scaledHeight * 0.34); //reduce 34%
        ...
        try {
            final BitmapEncoder encoder = BitmapEncoderRegistry.getInstance(mime);
            encoder.encode(scaledBI, out, mime, dpi);
        } finally {
            out.close();
        }
